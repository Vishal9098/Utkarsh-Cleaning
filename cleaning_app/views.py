...
def generate_bill(request, order_id):
    order = get_object_or_404(Order, order_id=order_id)
    # Invoice auto create karo agar nahi hai
    invoice, created = Invoice.objects.get_or_create(order=order)
    
    template = get_template('cleaning_app/invoice.html')
    html = template.render({'invoice': invoice, 'order': order})

    response = HttpResponse(content_type='application/pdf')
    response['Content-Disposition'] = f'attachment; filename="Bill-{invoice.invoice_number}.pdf"'
